...
A profile declares the wiki root and its policies, where task state lives,
which paths are generated, the document class globs, and the mode that governs
a path. A repository with one product puts all of that in a single file at the
