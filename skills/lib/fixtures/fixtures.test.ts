...
describe("a four-product repo cloned once per product", () => {
    async function index() {
        const root = await load("multi-product/project-profile.yaml", "root");
        const products = [
            await load(
                "multi-product/docs/notes-app/project-profile.yaml",
                "product",
            ),
            await load(
                "multi-product/docs/quiz/project-profile.yaml",
                "product",
            ),
        ];
     ...
     * quiz-arcade commit touching `packages/analysis` was reported as
    ...
    test("a mature package inside a greenfield product keeps mature", async () => {
        const { i } = await index();
        expect(modeForPath(i, "apps/notes-app/src/app.tsx")).toBe(
            "greenfield",
        );
        expect(modeForPath(i, "packages/notes-domain/src/rules.ts")).toBe(
            "mature",
        );
    });
    ...
    test("each product carries its own docs root and roadmap", async () => {
        const { i } = await index();
        const baby = productForPath(i, "apps/notes-app/src/app.tsx");
        expect(baby?.roadmap).toBe("docs/notes-app/milestones.md");
        expect(baby?.docs?.root).toBe("docs/notes-app");

        const quiz = productForPath(i, "apps/quiz/src/index.ts");
        expect(quiz?.roadmap).toBeUndefined();
    });
});

describe("a mature single-product repo, dual wiki profiles, no roadmap", () => {
    test("parses, and declares no roadmap", async () => {
        const profile = await load(
            "mature-single-product/project-profile.yaml",
            "root",
        );
